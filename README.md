...
7. [오류가 나면](#오류가-나면)
8. [조회 팁](#조회-팁)
9. [테이블 사전과 목적 검색](#테이블-사전과-목적-검색)
10. [에이전트에서 쓰기](#에이전트에서-쓰기)
11. [주의사항](#주의사항)
12. [개발](#개발)
...
**처음 사용하는 분은 [노트북 전용 상세 설명서](NOTEBOOK_GUIDE.md)를 먼저 읽으세요.** 설치·커널 선택부터 0~8절별 입력 예시, 결과 해석, CSV/Excel 저장, 오류 해결과 공유 전 점검까지 설명합니다. 아래는 빠른 시작 요약입니다.

`hhhs_db_manager.ipynb` 는 입력창으로 조건을 받아 조회하는 **DB 검색기** 형태입니다. 셀을 실행하면 테이블명 · 컬럼 · 조건 · 정렬 · 건수 등을 물어보고, 비워 두면 입력창 `[ ]` 안의 값으로 실행됩니다. 앞 셀에서 쓴 테이블명 · 컬럼명 · 건수는 뒷 셀 기본값으로 이어져 같은 값을 다시 입력하지 않아도 됩니다 (조건 · 정렬은 기억하지 않아 비우면 '조건 없음'입니다).
...
ERP 에 데이터 있는 테이블이 777개인데 ERP 자체 사전(`PIMS_TABLE_INFO`)과 `MS_Description` 에 이름이라도 있는 건 420개뿐이고, 한일합섬 커스텀(`Z_HISF`) 테이블은 대부분 설명이 없습니다. 이 도구는 **외부 LLM 을 부르지 않습니다.** 판단에 필요한 근거만 모아 주고, 판단은 사람 또는 팀원 PC 의 에이전트(Claude Code 등)가 해서 사전 파일에 적습니다.

사전 파일 `테이블사전.md` 는 접속정보처럼 **저장소에 올리지 않습니다** (`.gitignore`). 위치는 `HHHS_DICT_FILE` → 현재 폴더 → `.env` 가 있는 폴더 순으로 찾습니다.

| 명령 | 하는 일 |
| --- | --- |
| `hhhs-db dict init` | 데이터 있는 테이블 전부를 사전에 등록. ERP 사전·MS_Description 이 있으면 한글명을 `원본` 으로 채움. 기존 행은 유지 |
| `hhhs-db describe 테이블 [--sample N]` | **근거 팩**: ERP 사전 · 컬럼 한글명 · MS_Description · PK · 같은 이름줄기의 이웃(HEAD/LINE 짝) · 이름이 닮은 ERP 화면 · 사전 현재 항목 |
| `hhhs-db dict add 테이블 --name-kr … --desc … --basis …` | 판단한 설명을 기록. 상태 기본 `LLM추정`, 담당자가 확인하면 `--status 실무확인`. 실무확인 행은 `--force` 없이는 덮어쓰지 않음 |
| `hhhs-db dict show [키워드] [--missing] [--status 없음]` | 사전 보기 · 설명 비어 있는 것 · 상태별 |
| `hhhs-db find "목적 키워드"` | 사전 한글명·설명, 테이블 이름, **컬럼 한글명**을 대조해 후보 테이블을 점수순으로. `근거` 열에 무엇이 맞았는지 나옴 |

사전 표의 열: `테이블 | 모듈 | 한글명 | 설명 | 근거 | 상태 | 갱신일`. 상태는 `없음`(소스 없음) · `원본`(ERP 사전/MS_Description 그대로) · `LLM추정`(에이전트 추정, 검토 필요) · `실무확인`(담당자 확인) 네 가지로, 추정과 확인을 섞지 않습니다.

**에이전트로 설명 만들기** (Claude Code 에 `hanil-db-query` 스킬이 있으면 자동): `describe` 로 근거 팩을 읽고 → 2~3문장으로 판단하고 → `dict add` 로 기록 → 사용자에게 설명·근거·확신도를 보고. 근거에 없는 내용은 '추정'이라고 적습니다.

**목적으로 찾기**: `find "수주 납기"` 처럼 업무 용어로 검색 → 상위 후보를 `describe` 로 확인. 사전에 설명이 쌓일수록 정확해집니다. 파이썬에서는 `find_tables()`, `describe()`, `describe_text()`, `dict_load()`, `dict_upsert()`.
